...
our($_sql) = mpSql->new;
	...
	# Connect to the MySQL Server
 	$_sql->connect('mysql', '10.100.100.104', 'mpdb', 'mpFj8*Qm159788', 'mediaplace');
...
{
	my($_self, $_content) = @_;
	my($_article)         = $_sql->query("SELECT article_id FROM articles WHERE content = ".$_sql->quote($_content));
